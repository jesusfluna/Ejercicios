...
public class HomeController {
	private String rutaLocal = System.getProperty("user.home") + "/Desktop/almacenGHTML/";
	...
	@RequestMapping(value = "/descargarTodo", method = RequestMethod.POST)
	public String descargarTodo(Locale locale, Model model,@RequestParam("imagenes") String[] listacb,HttpServletResponse response) {
		logger.info("Welcome home! The client locale is {}.", locale);
		...
		if(listacb.length<1) {
			...
			model.addAttribute("message", "No se encontraron imagenes para descargar." );
		}else {
			int cont=1;
			
			File directorio=new File(rutaLocal); 
			directorio.mkdir();
			
			for (String string : listacb) {
				utilidades.descargar(string, rutaLocal, "imagen"+cont);
				cont++;
			}
			utilidades.generarHtml(rutaLocal);
			utilidades.makeZip(rutaLocal);
			
			try {
				InputStream is = new FileInputStream(rutaLocal.substring(0, rutaLocal.length()-1)+".zip");
			    org.apache.commons.io.IOUtils.copy(is, response.getOutputStream());
			    response.getOutputStream().flush();
			    response.getOutputStream().close();
			    response.flushBuffer();
			    
			    utilidades.deleteFolder(new File(rutaLocal));
			    utilidades.deleteFolder(new File(rutaLocal.substring(0, rutaLocal.length()-1)+".zip"));
			    
			    
				model.addAttribute("error", 0 );
				model.addAttribute("message", "La descarga ha ido realizada" );
			}catch(Exception e){
//				System.out.println("ERROR: "+e.getMessage());Esto de momento no funciona correctamente
				model.addAttribute("error", 1 );
				model.addAttribute("message", "No se pudieron descargar las imagenes." );
			}
		}
		return "home";
	}
